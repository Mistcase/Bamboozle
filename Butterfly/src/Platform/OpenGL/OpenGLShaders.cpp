...
namespace butterfly
{
    ...
    Shader* OpenGLShaders::createFromFile(const std::filesystem::path& filepath)
    {
        ...
        if (instance != nullptr)
        {
            ...
            BUTTERFLY_CORE_TRACE("Shader {0} loaded ({1})", name, filepath.string());
        }
    ...
    Shader* OpenGLShaders::get(uint32_t id)
    {
        ...
        if (it == m_shaders.end())
        {
            BUTTERFLY_CORE_INFO("Shader {0} is not found", id);
            return nullptr;
        }
    ...
    std::string OpenGLShaders::loadSource(const std::filesystem::path& filepath) const
    {
        ...
        if (!fs.is_open())
        {
            BUTTERFLY_CORE_ERROR("Cannot open shader source file {0}", strPath);
            return source;
        }
        ...
        BUTTERFLY_CORE_TRACE("Shader file opened: size={0}, ({1})", fsz, strPath);
